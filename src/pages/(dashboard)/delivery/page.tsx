...
const DeliveryPage = () => {
    ...
    const { deliveryDelete } = deliveryMutations()
    ...
    const handleDeleteDelivery = (id: any) => {
        const confirmDelete = window.confirm(
            "Bạn có chắc muốn xóa nhà xe này không?"
        );

        if (!confirmDelete) return;
        deliveryDelete.mutate(id)
    }
    return (
        <div className="overflow-x-auto rounded-[10px] border border-gray-200 px-[10px]">

            <div className="justify-between flex items-center my-[10px]">
                <Link to={`/order`} className="">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M10.5 19.5 3 12m0 0 7.5-7.5M3 12h18" />
                    </svg>
                </Link>
                <Link to={`/delivery/add`} className='bg-blue-500 p-[5px_15px] text-white rounded-sm'>Thêm</Link>
            </div>
                        ...
                        <tr className="border-t hover:bg-gray-50 *:p-[5px]">
                            ...
                            <td className="p-[12px]">
                                <div className="flex items-center justify-center gap-[10px] *:cursor-pointer">
                                    ...
                                    <div onClick={() => handleDeleteDelivery(item._id)} className="">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
