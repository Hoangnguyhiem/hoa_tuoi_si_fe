...
const CategoryPage = () => {
    ...
    const { data: categories } = useQuery({
        queryKey: ["categories"],
        queryFn: async () => {
            return await categoriesSevices.listAllCategories()
        }
    })

    const handleDeleteCategories = (id: any) => {
            ...
            <div className="border-slate-200 border-[1px]">
                <div className="fixed mx-[10px] left-0 right-0 bg-slate-200 px-[10px]">
                    <div className="flex justify-between items-center h-[60px] font-[700]">
                        ...
                        <Link to={`/categories/add`} className='bg-blue-500 p-[5px_15px] text-white rounded-sm'>Thêm</Link>
                    </div>
                </div>
